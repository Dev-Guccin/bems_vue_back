...
function modbusStart() {
    for (let i = 0; i < Networks.length; i++) { // 소켓을 설정하고 열어준다.
                    ...
                    for (let fi = 0; fi < targetchannels.length; fi++) {//frame의 개수만큼 반복하는 코드
                        ...
                        if (targetchannels[fi].active == 1) { // active 상태일때만 반복시킴
                            ...
                            switch (targetchannels[fi].function_code) {
                                case 0://Read Coils
                                    func = clients[i].readCoils(targetchannels[fi].start_address, targetchannels[fi].quantity)
                                    break
                                case 1://Read Discrete Input
                                    func = clients[i].readDiscreteInputs(targetchannels[fi].start_address, targetchannels[fi].quantity)
                                    break
                                case 3://Read Holding Registers
                                    func = clients[i].readHoldingRegisters(targetchannels[fi].start_address, targetchannels[fi].quantity)
                                    break
                                case 4://Read Input Registers
                                    func = clients[i].readInputRegisters(targetchannels[fi].start_address, targetchannels[fi].quantity)
                                    break
                            }
            ...
            sockets[i].on('open', async function () {
                ...
                for (let fi = 0; fi < targetchannels.length; fi++) { // socket과 slave_id를 통해 clients를 열어준다.
                    ...
                    if (targetchannels[fi].active == 1) { // active 상태일때만 반복시킴
                        switch (targetchannels[fi].function_code) {
                            case 0://Read Coils
                                func = rtu_clients[i][slave_id].readCoils(targetchannels[fi].start_address, targetchannels[fi].quantity)
                                break
                            case 1://Read Discrete Input
                                func = rtu_clients[i][slave_id].readDiscreteInputs(targetchannels[fi].start_address, targetchannels[fi].quantity)
                                break
                            case 3://Read Holding Registers
                                func = rtu_clients[i][slave_id].readHoldingRegisters(targetchannels[fi].start_address, targetchannels[fi].quantity)
                                break
                            case 4://Read Input Registers
                                func = rtu_clients[i][slave_id].readInputRegisters(targetchannels[fi].start_address, targetchannels[fi].quantity)
                                break
                        }
                    }
                    DBH.channel_inc('tx', targetchannels[fi].id)
            ...
            switch (sensors[se].m_dattype) {
                ...
                case 14://14 : 1bit value
                    ...
                    if (targetchannels_fi.function_code == 3 || targetchannels_fi.function_code == 4) { //아날로그로 읽는 경우
                        if (modbus_result.readInt16BE(targetIdx) & (1 << (15 - sensors[se].m_bitoffset))) {
                            resData = 1
                        } else { resData = 0 }
                    } else {                                                                              //바이너리로 읽어오는 경우
                        if (modbus_result.readInt8(parseInt((sensors[se].m_bitoffset + 1) / 8)) & (1 << (sensors[se].m_bitoffset + 1) % 8 - 1)) resData = 1
                        else resData = 0
                    }
                    break;
            }
